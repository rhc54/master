        ...
        if (NULL != (start_pos = strstr(retval, "${" #fieldname "}"))) { \
            ...
            *start_pos = '\0';                                          \
            ...
            asprintf(&retval, "%s%s%s", tmp,                            \
                     pmix_install_dirs.ompiname + destdir_offset,       \
                     end_pos);                                          \
            free(tmp);                                                  \
            changed = true;                                             \
 ...
 */
static char *
pmix_install_dirs_expand_internal(const char* input, bool is_setup)
{
    size_t len, i;
...
char *
pmix_install_dirs_expand(const char* input)
{
    /* We do NOT want PMIX_DESTDIR expansion in this case. */
    return pmix_install_dirs_expand_internal(input, false);
}


char *
pmix_install_dirs_expand_setup(const char* input)
{
    /* We DO want PMIX_DESTDIR expansion in this case. */
    return pmix_install_dirs_expand_internal(input, true);
}
