...
static pmix_hash_table_t mca_base_component_repository;
...
static int process_repository_item (const char *filename, void *data)
{
    ...
    if (PMIX_SUCCESS != ret) {
        ...
        ret = pmix_hash_table_set_value_ptr (&mca_base_component_repository, type,
                                             strlen (type), (void *) component_list);
        if (PMIX_SUCCESS != ret) {
            free (base);
    ...
    PMIX_LIST_FOREACH(ri, component_list, mca_base_component_repository_item_t) {
...
int mca_base_component_repository_add (const char *path)
{
    ...
    do {
        ...
        if (0 != pmix_dl_foreachfile(dir, process_repository_item, NULL)) {
            break;
        }
...
int mca_base_component_repository_init(void)
{
  ...
  if (!initialized) {
    ...
    int ret = mca_base_framework_open(&pmix_dl_base_framework, 0);
    if (PMIX_SUCCESS != ret) {
...
#if PMIX_HAVE_DL_SUPPORT
    return pmix_hash_table_get_value_ptr (&mca_base_component_repository, framework->framework_name,
                                        ...
                                        mca_base_component_repository_item_t *ri)
{
    ...
    pmix_output_verbose(MCA_BASE_VERBOSE_INFO, 0, "mca_base_component_repository_open: examining dynamic "
    ...
    PMIX_LIST_FOREACH(mitem, &framework->framework_components, mca_base_component_list_item_t) {
        if (0 == strcmp(mitem->cli_component->mca_component_name, ri->ri_name)) {
            pmix_output_verbose (MCA_BASE_VERBOSE_INFO, 0, "mca_base_component_repository_open: already loaded (ignored)");
    ...
    char *err_msg = NULL;
    if (PMIX_SUCCESS != pmix_dl_open(ri->ri_path, true, false, &ri->ri_dlhandle, &err_msg)) {
        if (NULL == err_msg) {
        ...
        pmix_output_verbose(vl, 0, "mca_base_component_repository_open: unable to open %s: %s (ignored)",
                            ri->ri_base, err_msg);
        return PMIX_ERR_BAD_PARAM;
    }
    ...
    do {
        ...
        err_msg = NULL;
        ret = pmix_dl_lookup(ri->ri_dlhandle, struct_name, (void**) &component_struct, &err_msg);
        if (PMIX_SUCCESS != ret || NULL == component_struct) {
            if (NULL == err_msg) {
                err_msg = "pmix_dl_loookup() error message was NULL!";
            }
            pmix_output_verbose(vl, 0, "mca_base_component_repository_open: \"%s\" does not appear to be a valid "
        ...
        if (!(MCA_BASE_VERSION_MAJOR == component_struct->mca_major_version &&
            ...
            pmix_output_verbose(vl, 0, "mca_base_component_repository_open: %s \"%s\" uses an MCA interface that is "
        ...
        pmix_output_verbose (MCA_BASE_VERBOSE_INFO, 0, "mca_base_component_repository_open: opened dynamic %s MCA "
...
void mca_base_component_repository_finalize(void)
{
    ...
    ret = pmix_hash_table_get_first_key_ptr (&mca_base_component_repository, &key, &key_size,
                                             (void **) &component_list, &node);
    while (PMIX_SUCCESS == ret) {
        PMIX_LIST_RELEASE(component_list);
        ret = pmix_hash_table_get_next_key_ptr (&mca_base_component_repository, &key,
