...
static pmix_list_t mca_base_var_file_values;
static pmix_list_t mca_base_envar_file_values;
static pmix_list_t mca_base_var_override_values;

static int mca_base_var_count = 0;

static pmix_hash_table_t mca_base_var_index_hash;
...
PMIX_CLASS_INSTANCE(mca_base_var_t, pmix_object_t,
...
PMIX_CLASS_INSTANCE(mca_base_var_file_value_t, pmix_list_item_t,
...
static char *append_filename_to_list(const char *filename)
{
    ...
    (void) pmix_argv_append_unique_nosize(&mca_base_var_file_list, filename, false);

    count = pmix_argv_count(mca_base_var_file_list);
...
int mca_base_var_init(void)
{
    ...
    if (!mca_base_var_initialized) {
        ...
        PMIX_CONSTRUCT(&mca_base_vars, pmix_pointer_array_t);
        /* These values are arbitrary */
        ret = pmix_pointer_array_init (&mca_base_vars, 128, 16384, 128);
        if (PMIX_SUCCESS != ret) {
            return ret;
        }
        ...
        PMIX_CONSTRUCT(&mca_base_var_file_values, pmix_list_t);
        PMIX_CONSTRUCT(&mca_base_envar_file_values, pmix_list_t);
        PMIX_CONSTRUCT(&mca_base_var_override_values, pmix_list_t);
        PMIX_CONSTRUCT(&mca_base_var_index_hash, pmix_hash_table_t);

        ret = pmix_hash_table_init (&mca_base_var_index_hash, 1024);
        if (PMIX_SUCCESS != ret) {
            return ret;
        }
        ...
        (void)mca_base_var_register ("pmix", "mca", "base", "env_list",
                                     "Set SHELL env variables",
                                     MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, PMIX_INFO_LVL_3,
                                     MCA_BASE_VAR_SCOPE_READONLY, &mca_base_env_list);

        mca_base_env_list_sep = MCA_BASE_ENV_LIST_SEP_DEFAULT;
        (void)mca_base_var_register ("pmix", "mca", "base", "env_list_delimiter",
        ...
        if (NULL != mca_base_env_list) {
            ...
            if (NULL != name) {
                pmix_setenv(name, mca_base_env_list, false, &environ);
                free(name);
            }
        }
         ...
         */
        (void)mca_base_var_register ("pmix", "mca", "base", "env_list_internal",
                "Store SHELL env variables from amca conf file",
                MCA_BASE_VAR_TYPE_STRING, NULL, 0, MCA_BASE_VAR_FLAG_INTERNAL, PMIX_INFO_LVL_3,
...
static int mca_base_var_cache_files(bool rel_path_search)
{
    ...
    asprintf(&mca_base_var_files, "%s"PMIX_PATH_SEP".openmpi" PMIX_PATH_SEP
             "mca-params.conf%c%s" PMIX_PATH_SEP "openmpi-mca-params.conf",
             home, PMIX_ENV_SEP, pmix_install_dirs.sysconfdir);
#else
    asprintf(&mca_base_var_files, "%s" PMIX_PATH_SEP "openmpi-mca-params.conf",
    ...
    ret = mca_base_var_register ("pmix", "mca", "base", "param_files", "Path for MCA "
                                 "configuration files containing variable values",
                                 MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, PMIX_INFO_LVL_2,
    ...
    (void) mca_base_var_register_synonym (ret, "pmix", "mca", NULL, "param_files",
                                          MCA_BASE_VAR_SYN_FLAG_DEPRECATED);

    ret = asprintf(&mca_base_var_override_file, "%s" PMIX_PATH_SEP "openmpi-mca-params-override.conf",
                   pmix_install_dirs.sysconfdir);
    if (0 > ret) {
        return PMIX_ERR_OUT_OF_RESOURCE;
    }
    ...
    ret = mca_base_var_register ("pmix", "mca", "base", "override_param_file",
                                 ...
                                 PMIX_INFO_LVL_2, MCA_BASE_VAR_SCOPE_CONSTANT,
    ...
    ret = mca_base_var_register ("pmix", "mca", "base", "suppress_override_warning",
                                 "Suppress warnings when attempting to set an overridden value (default: false)",
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, PMIX_INFO_LVL_2,
    ...
    ret = mca_base_var_register ("pmix", "mca", "base", "param_file_prefix",
    ...
    ret = mca_base_var_register ("pmix", "mca", "base", "envar_file_prefix",
    ...
    ret = asprintf(&mca_base_param_file_path, "%s" PMIX_PATH_SEP "amca-param-sets%c%s",
                   pmix_install_dirs.pmixdatadir, PMIX_ENV_SEP, cwd);
    if (0 > ret) {
        return PMIX_ERR_OUT_OF_RESOURCE;
    }

    tmp = mca_base_param_file_path;
    ret = mca_base_var_register ("pmix", "mca", "base", "param_file_path",
    ...
    force_agg_path = NULL;
    ret = mca_base_var_register ("pmix", "mca", "base", "param_file_path_force",
    ...
    if (NULL != force_agg_path) {
        if (NULL != mca_base_param_file_path) {
            ...
            asprintf(&mca_base_param_file_path, "%s%c%s", force_agg_path, PMIX_ENV_SEP, tmp_str);
            free(tmp_str);
        } else {
    ...
    if (NULL != mca_base_var_file_prefix) {
       resolve_relative_paths(&mca_base_var_file_prefix, mca_base_param_file_path, rel_path_search, &mca_base_var_files, PMIX_ENV_SEP);
    }
    read_files (mca_base_var_files, &mca_base_var_file_values, PMIX_ENV_SEP);
    ...
    if (0 == access(mca_base_var_override_file, F_OK)) {
        read_files (mca_base_var_override_file, &mca_base_var_override_values, PMIX_ENV_SEP);
    }
...
static int var_get (int vari, mca_base_var_t **var_out, bool original)
{
    ...
    var = pmix_pointer_array_get_item (&mca_base_vars, vari);
    if (NULL == var) {
        return PMIX_ERR_BAD_PARAM;
    }
...
static int var_find_by_name (const char *full_name, int *vari, bool invalidok)
{
    ...
    rc = pmix_hash_table_get_value_ptr (&mca_base_var_index_hash, full_name, strlen (full_name),
                                        &tmp);
    if (PMIX_SUCCESS != rc) {
        return rc;
    }
...
int mca_base_var_set_flag (int vari, mca_base_var_flag_t flag, bool set)
{
    ...
    ret = var_get (vari, &var, true);
    if (PMIX_SUCCESS != ret || VAR_IS_SYNONYM(var[0])) {
        return PMIX_ERR_BAD_PARAM;
    }
...
int mca_base_var_build_env(char ***env, int *num_env, bool internal)
{
    ...
    len = pmix_pointer_array_get_size(&mca_base_vars);
    for (i = 0; i < len; ++i) {
        char *value_string;
        char *str = NULL;

        var = pmix_pointer_array_get_item (&mca_base_vars, i);
        if (NULL == var) {
            continue;
        }
...
int mca_base_var_finalize(void)
{
    ...
    if (mca_base_var_initialized) {
        size = pmix_pointer_array_get_size(&mca_base_vars);
        for (i = 0 ; i < size ; ++i) {
            pmixect = pmix_pointer_array_get_item (&mca_base_vars, i);
            if (NULL != pmixect) {
                PMIX_RELEASE(pmixect);
            }
        }
        PMIX_DESTRUCT(&mca_base_vars);

        while (NULL !=
               (item = pmix_list_remove_first(&mca_base_var_file_values))) {
            PMIX_RELEASE(item);
        }
        PMIX_DESTRUCT(&mca_base_var_file_values);

        while (NULL !=
               (item = pmix_list_remove_first(&mca_base_envar_file_values))) {
            PMIX_RELEASE(item);
        }
        PMIX_DESTRUCT(&mca_base_envar_file_values);

        while (NULL !=
               (item = pmix_list_remove_first(&mca_base_var_override_values))) {
            PMIX_RELEASE(item);
        }
        PMIX_DESTRUCT(&mca_base_var_override_values);
        ...
        if (NULL != mca_base_var_file_list) {
            pmix_argv_free(mca_base_var_file_list);
        }
        mca_base_var_file_list = NULL;

        (void) mca_base_var_group_finalize ();

        PMIX_DESTRUCT(&mca_base_var_index_hash);
                              ...
                              void *storage)
{
    ...
    if (flags & MCA_BASE_VAR_FLAG_SYNONYM) {
        original = pmix_pointer_array_get_item (&mca_base_vars, synonym_for);
        if (NULL == original) {
    ...
    if (0 > var_index) {
        ...
        var_index = pmix_pointer_array_add (&mca_base_vars, var);
        if (0 > var_index) {
            PMIX_RELEASE(var);
            return PMIX_ERROR;
        }
        ...
        pmix_hash_table_set_value_ptr (&mca_base_var_index_hash, var->mbv_full_name, strlen (var->mbv_full_name),
 ...
 */
static int var_set_from_file (mca_base_var_t *var, mca_base_var_t *original, pmix_list_t *file_values)
{
    const char *var_full_name = var->mbv_full_name;
    const char *var_long_name = var->mbv_long_name;
    bool deprecated = VAR_IS_DEPRECATED(var[0]);
    ...
    PMIX_LIST_FOREACH(fv, file_values, mca_base_var_file_value_t) {
