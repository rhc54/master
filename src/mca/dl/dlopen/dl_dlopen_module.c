...
static int dlopen_lookup(pmix_dl_handle_t *handle, const char *symbol,
...
static int dlopen_close(pmix_dl_handle_t *handle)
{
    assert(handle);
                              ...
                              void *data)
{
    ...
    dirs = pmix_argv_split(search_path, OPAL_ENV_SEP);
 ...
 * Module definition
 */
pmix_dl_base_module_t pmix_dl_dlopen_module = {
    .open = dlopen_open,
