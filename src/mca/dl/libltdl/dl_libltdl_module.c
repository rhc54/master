                       ...
                       pmix_dl_handle_t **handle, char **err_msg)
{
    ...
    *handle = NULL;
    ...
    pmix_dl_libltdl_component_t *c = &mca_dl_libltdl_component;
    ...
    if (NULL != local_handle) {
        *handle = calloc(1, sizeof(pmix_dl_handle_t));
...
static int libltdl_lookup(pmix_dl_handle_t *handle, const char *symbol,
...
static int libltdl_close(pmix_dl_handle_t *handle)
{
    assert(handle);
 ...
 * Module definition
 */
pmix_dl_base_module_t pmix_dl_libltdl_module = {
    .open = libltdl_open,
