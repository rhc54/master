...
pmix_pnet_module_t pmix_cornelis_module = {
    .name = "cornelis",
    .allocate = allocate,
                              ...
                              pmix_list_t *ilist)
{
    ...
    pmix_output_verbose(2, pmix_pnet_base_framework.framework_output,
                        "pnet:cornelis:allocate for nspace %s", nptr->nspace);
    ...
    if (seckeys) {
        pmix_output_verbose(2, pmix_pnet_base_framework.framework_output,
                            "pnet: cornelis providing seckeys");
    ...
    if (envars) {
        pmix_output_verbose(2, pmix_pnet_base_framework.framework_output,
                            "pnet: cornelis harvesting envars %s excluding %s",
                            (NULL == pmix_mca_pnet_cornelis_component.incparms)
                                ? "NONE"
                                : pmix_mca_pnet_cornelis_component.incparms,
                            (NULL == pmix_mca_pnet_cornelis_component.excparms)
                                ? "NONE"
                                : pmix_mca_pnet_cornelis_component.excparms);
        /* harvest envars to pass along */
        PMIX_CONSTRUCT(&cache, pmix_list_t);
        if (NULL != pmix_mca_pnet_cornelis_component.include) {
            rc = pmix_util_harvest_envars(pmix_mca_pnet_cornelis_component.include,
                                          pmix_mca_pnet_cornelis_component.exclude, &cache);
            if (PMIX_SUCCESS != rc) {
                                         ...
                                         pmix_info_t info[], size_t ninfo)
{
    ...
    pmix_output_verbose(2, pmix_pnet_base_framework.framework_output,
                        "pnet:cornelis:setup_local with %lu info", (unsigned long) ninfo);
    ...
    for (n = 0; n < ninfo; n++) {
        ...
        if (PMIX_CHECK_KEY(&info[n], PMIX_PNET_OPA_BLOB)) {
            pmix_output_verbose(2, pmix_pnet_base_framework.framework_output,
                                "pnet:cornelis:setup_local found my blob");
...
static pmix_status_t collect_inventory(pmix_info_t directives[], size_t ndirs,
                                       pmix_list_t *inventory)
{
    /* search the topology for Cornelis NICs */
    PMIX_HIDE_UNUSED_PARAMS(directives, ndirs, inventory);


    /* This function is executed on remote daemons to scan their local
     * topology and extract only the Cornelis devices for return. The
     * results are passed back to you via the "deliver_inventory" call
     * below.
     */
                                       ...
                                       pmix_info_t directives[], size_t ndirs)
{
    ...
    /* Primary objective here is to create a list of all devices
     * in the allocation - only the controller (e.g., mpirun) executes
     * this. You can then use the list to, for example, assign static
     * endpoints to avoid exchanging them at startup
     *
     * Note that you don't have to implement this - it is simply
     * available should you wish to support it.
     */
