...
PMIX_CLASS_INSTANCE(cmd_line_param_t,
                    pmix_list_item_t,
...
static cmd_line_option_t *find_option(pmix_cmd_line_t *cmd,
 ...
 * handle.
 */
int pmix_cmd_line_parse(pmix_cmd_line_t *cmd, bool ignore_unknown,
                        int argc, char **argv)
{
    int i, j, orig, ret;
...
char *pmix_cmd_line_get_usage_msg(pmix_cmd_line_t *cmd)
{
    ...
    for (i = 0, item = pmix_list_get_first(&cmd->lcl_options);
         pmix_list_get_end(&cmd->lcl_options) != item;
         ++i, item = pmix_list_get_next(item)) {
...
int pmix_cmd_line_get_ninsts(pmix_cmd_line_t *cmd, const char *opt)
{
    ...
    if (NULL != option) {
        for (item = pmix_list_get_first(&cmd->lcl_params);
             pmix_list_get_end(&cmd->lcl_params) != item;
...
static void cmd_line_constructor(pmix_cmd_line_t *cmd)
{
    ...
    OBJ_CONSTRUCT(&cmd->lcl_options, pmix_list_t);
    OBJ_CONSTRUCT(&cmd->lcl_params, pmix_list_t);
                                      ...
                                      const char *option_name)
{
    ...
    for (item = pmix_list_get_first(&cmd->lcl_options);
         pmix_list_get_end(&cmd->lcl_options) != item;
         item = pmix_list_get_next(item)) {
